...
- `GET /todos/workshop-score?category=Estudos` — endpoint didático com lógica duplicada
- `GET /todos/legacy-search?pattern=API` — endpoint didático de busca insegura
- `GET /todos/diagnostic-token?user=aluno` — endpoint didático com token fraco
- `GET /todos/:id/archive-status` — endpoint didático com condição redundante
- `GET /todos/workshop/statistics` — estatísticas com caso extremo não tratado
- `GET /todos/workshop/priority-ranking` — ranking com ordenação numérica incorreta
- `GET /todos/workshop/html-list` — relatório HTML sem escaping
- `GET /todos/workshop/html-cards` — variante duplicada do relatório HTML
- `PATCH /todos/workshop/bulk-complete?ids=1,2` — conclusão em lote ineficiente
- `GET /todos/workshop/manager-summary` — relatório com bloco propositalmente duplicado
- `GET /todos/workshop/supervisor-summary` — cópia literal do relatório gerencial
